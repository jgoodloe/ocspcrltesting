...
def run_crl_tests(
    ...
    if should_run("CRL download and parsing from certificate CRL Distribution Points"):
    ...
    if should_run("CRL Distribution Point accessibility"):
    ...
    if should_run("CRL signature verification"):
    ...
    if should_run("CRL timestamp validation"):
    ...
    if should_run("Certificate revocation status check"):
    ...
    if should_run("CRL vs OCSP consistency check"):
