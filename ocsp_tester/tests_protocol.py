...
def run_protocol_tests(ocsp_url: str, issuer: x509.Certificate, leaf: x509.Certificate) -> List[TestCaseResult]:
    ...
    # 1. HTTP POST
    if should_run("HTTP POST transport"):
    ...
    if should_run("DER encoding and basic response fields"):
    ...
    if should_run("CertID SHA-1 for issuer hashes"):
    ...
    if should_run("Serial number handling"):
