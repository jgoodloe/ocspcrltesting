...
def run_federal_tests(
    ...
    if any_selected("Federal PKI environment detection", "Batch OCSP response handling (DHS CA4 style)", "Delegated responder EKU (id-kp-OCSPSigning)", "Response validity interval (freshness)"):
        try:
    ...
    if should_run("Federal PKI environment detection"):
    ...
    if should_run("Batch OCSP response handling (DHS CA4 style)"):
    ...
    if should_run("OCSP signer signature verification (trust chain)"):
    ...
    if should_run("Delegated responder EKU (id-kp-OCSPSigning)"):
    ...
    if should_run("Response validity interval (freshness)"):
